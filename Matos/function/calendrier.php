...
class Calendar
{
    ...
    private function _showDay($cellNumber)
    {
        ...
        foreach ($lockedDates as $lockedDate) {
            if (date('Y-m-d', strtotime($lockedDate->getDateDebut())) == $this->currentDate) {
                $state = "<p style='font-size: 80%;background-color: #ff0000'>Réservé</p>";
            }
            if (date('Y-m-d', strtotime($lockedDate->getDateFin())) == $this->currentDate) {
                $state = "<p style='font-size: 80%;background-color: #ff0000'>Réservé</p>";
            }
        ...
        return '<li id="li-' . $this->currentDate . '" class="' . ($cellNumber % 7 == 1 ? ' start ' : ($cellNumber % 7 == 0 ? ' end ' : ' ')) .
            ($cellContent == null ? 'mask' : '') . '">' . $state . '</li>';
    }
    ...
    private function _createNavi()
    {
        ...
        return
            '<div class="header">' .
            '<a class="prev" href="index.php?uc=calendriers&idMateriel=' . $_GET['idMateriel'] . '&month=' . sprintf('%02d', $preMonth) . '&year=' . $preYear . '">&#11013;&#65039;</a>' .
            '<span class="title">' . date('m / Y', strtotime($this->currentYear . '-' . $this->currentMonth . '-1')) . '</span>' .
            '<a class="next" href="index.php?uc=calendriers&idMateriel=' . $_GET['idMateriel'] . '&month=' . sprintf("%02d", $nextMonth) . '&year=' . $nextYear . '">&#10145;&#65039;</a>' .
            '</div>';
    }
